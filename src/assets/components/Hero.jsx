...
const Hero = ({searchQuery}) => {
    ...
    useEffect(() =>{
        const fetchBooks = async() =>{
            try {
                setLoading(true);
                const response = await fetch(`https://www.googleapis.com/books/v1/volumes?q=${encodeURIComponent(searchQuery)}&maxResult=35=free-ebooks&key=AIzaSyAuPRREeOHpZaiLfiFRXR57gH454TWzcr8`)
                ...
                conlsole.error('Error Fetching Books:', error);
    ...
    const generatePageNumer = () => [1,2,3,4,5,6];
        ...
        <div className=' max-w-7xl mx-auto'>
            <div className=' grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 lg:gap-8'>
                {paginatedBooks.map((book, index) => {
                    ...
                    return(
                        ...
                        backdrop-blur-xl border border-white/10 hover:border-cyan-400/30 transition-all duration-300 hover:-translate-y-2
                        shadow-lg hover:shadow-xl md:shadow-2xl hover:shadow-cyan-400/10'>
                            ...
                            className=' absolute inset-0 z-10 cursor-pointer' 
                             ...
                             <div className=' mt-4 md:mt-6'>
                                <h3 className=' text-lg md:text-xl font-bold px-4 text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-blue-400'>
                                    {book.title}
                                </h3>
                                <p className=' text-gray-400 mt-1 md:mt-2 px-4 text-sm md:text-base'>
                                    {book.authors}
                                </p>

                                <div className=' flex items-center  mt-2  md:mt-3'>
                                    <div className=' flex  text-amber-400  p-4'>
                                        {[...Array(5)].map((_, i) =>(
                                            <IoStarSharp key={i} className={` h-4 w-4 md:h-5 md:w-5
                                ...
                                <div className=' grid p-4 grid-cols-2 gap-2 md:gap-4 md:mt-4 text-xs md:text-sm'>
                                ...
                                <div className=' mt-2 p-2 md:mt-4 flex flex-wrap gap-1 md:gap-2'>
            ...
            <div className=' mt-6 md:mt-8 flex flex-wrap justify-center gap-2 md:gap-3'>
                <button onClick={() => setCurrentPage(p => Math.max(1, p-1))}
                    ...
                    {generatePageNumer().map((page, index) => (
